 ...
 *  beginning of the next line (scrolling if necessary).  If the character
 *  is a carriage return ('\r'), the cursor is immediately reset to the
 *  beginning of the current line, causing any future output to overwrite
 *  any existing output on the line.  If backsapce ('\b') is encountered,
 ...
 *  len is not a positive integer or s is null, the function has no effect.
 *
 ...
 *
 *  @param color The address to which the current color information will be
 *  written.
 *
 *  @return Void.
 */
 ...
 *
 *  Subsequent calls to putbytes should cause the console output to begin
 ...
 *
 *  Subsequent calls to putbytes do not cause the cursor to show again.
 *
 *  @return Void.
 */
void hide_cursor();
...
/** @brief Prints character ch with the specified color at position (row,
 * col).
 *
